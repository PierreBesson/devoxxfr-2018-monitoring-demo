...
class PaymentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Payment entity")
        ...
        .repeat(2) {
            ...
            .get("/booking/api/payments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new payment")
            .post("/booking/api/payments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "price":"0", "buyerName":"SAMPLE_TEXT", "notes":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_payment_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created payment")
                .get("/booking${new_payment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created payment")
            .delete("/booking${new_payment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
